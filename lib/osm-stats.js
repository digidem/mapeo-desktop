var Index = require('hyperdb-index-level')
var sub = require('subleveldown')

module.exports = installStatsIndex

function installStatsIndex (osm) {
  var idb = sub(osm.index, 'stat')
  var idx = Index(osm.db, idb, map)

  function map (node, next) {
    if (!node.value.deleted && node.value.type === 'node') {
      var elm = node.value
      // console.log('indexing', node.key)
      getIndexState(function (err, stats) {
        if (err) return next(err)
        stats.latSum += Number(elm.lat)
        stats.lonSum += Number(elm.lon)
        stats.numNodes++
