...
var stratify = require('./lib/stratify-geojson-layers')
      ...
      submenu: [
        {
          ...
          click: clicked_importConfiguration
        },
        {
          label: 'Export as GeoJSON..',
          click: clicked_exportGeoJSON,
          visible: true
        }
      ]
    },
...
function clicked_importConfiguration (item, focusedWindow) {
...
function clicked_exportGeoJSON (item, focusedWindow) {
  dialog.showSaveDialog({
    ...
    collect(ex, function (err, data) {
      ...
      var layers = stratify(data)
      for (var name in layers) {
        var filename = path.join(root, name + '.geojson')
        fs.writeFileSync(filename, layers[name])
      }
