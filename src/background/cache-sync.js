...
const middleWare = (module.exports = function (getAsset, options) {
  return async function (req, params, cb) {
    options = options || {}
    options.cacheDir =
    ...
    try {
      ...
      if (fs.existsSync(assetCachePath)) {
        ...
        const filepath = `${assetCachePath}/${firstFile}`

        // touch file for LRU eviction
        middleWare.touch(filepath)

        const [type, size] = middleWare.decodeAssetCacheName(
          firstFile
        )

        const buffer = fs.readFileSync(filepath)
        info = {type, size, buffer}
        ...
        info = await getAsset(params)
        const fileName = middleWare.encodeAssetCacheName(info.type, info.size)
        ...
        fs.writeFileSync(`${assetCachePath}/${fileName}`, info.buffer)
